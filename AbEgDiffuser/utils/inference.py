...
class RemoveNative(object):
    ...
    def __call__(self, data):
        ...
        if self.remove_sequence:
            ...
            data['atom_types'] = torch.where(
                generate_flag[:, None].expand(data['atom_types'].shape),
                torch.full_like(data['atom_types'], fill_value=int(constants.atomsymb_to_atomindex[constants.atom_unk])),  # Is loop
                data['atom_types']
            )
            data['atom_positions'] = torch.where(
                generate_flag[:, None].expand(data['atom_positions'].shape),
                torch.full_like(data['atom_positions'],
                                fill_value=int(constants.possymb_to_posindex[constants.atom_pos_unk])),  # Is loop
                data['atom_positions']
            )
