...
from AbEgDiffuser.datasets.custom import preprocess_antibody_structure
from AbEgDiffuser.models import get_model
from AbEgDiffuser.modules.common.geometry import reconstruct_backbone_partially
from AbEgDiffuser.modules.common.so3 import so3vec_to_rotation
from AbEgDiffuser.utils.inference import RemoveNative
from AbEgDiffuser.utils.protein.writers import save_pdb
from AbEgDiffuser.utils.train import recursive_to
from AbEgDiffuser.utils.misc import *
from AbEgDiffuser.utils.data import *
from AbEgDiffuser.utils.transforms import *
from AbEgDiffuser.utils.inference import *
from AbEgDiffuser.tools.renumber import renumber as renumber_antibody
    ...
    for variant in data_variants:
        ...
        for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
            ...
            pos_atom_new, mask_atom_new = reconstruct_backbone_partially(
                pos_ctx = batch['pos_heavyatom'], # (N, L, A, 3)
                R_new = so3vec_to_rotation(traj_batch[0][0]), # (N, L, 3, 3)
                t_new = traj_batch[0][1], # (N, L, 3)
                aa = aa_new, # (N, L)
                chain_nb = batch['chain_nb'], # (N, L)
                res_nb = batch['res_nb'], # (N, L)
                mask_atoms = batch['mask_heavyatom'], # (N, L, A)
                mask_recons = batch['generate_flag'], # (N, L)
            )
