...
class TaskScanner:
    ...
    def __init__(self, root, final_postfix=None):
        ...
        self.failed_log = os.path.join(root, "_failed_tasks.log")
    ...
    def scan(self) -> List[RelaxTask]: 
        ...
        # 过滤已经记录为失败的任务
        failed_paths = self._load_failed_tasks()
        tasks = [t for t in tasks if t.in_path not in failed_paths]
