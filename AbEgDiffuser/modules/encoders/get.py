...
from AbEgDiffuser.modules.common.nn_utils import _unit_edges_from_block_edges, stable_norm
from AbEgDiffuser.modules.common.radial_basis import RadialBasis


class GET(nn.Module):
    '''Generalist Equivariant Transformer'''

    def __init__(self, d_hidden, d_edge=16, n_layers=4, d_radial=16, n_channel=1, n_rbf=16, cutoff=7.0, n_head=4,
...
class GETLayer(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0,
        ...
        nn.init.orthogonal_(self.linear_qk.weight)  # zyb修改 正交初始化
        self.linear_v = nn.Linear(d_hidden_head, d_radial_head)
        nn.init.xavier_normal_(self.linear_v.weight)  # zyb修改 Xavier初始化
