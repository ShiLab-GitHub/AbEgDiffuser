...
from AbEgDiffuser.modules.common.geometry import construct_3d_basis, global_to_local, get_backbone_dihedral_angles
from AbEgDiffuser.modules.common.layers import AngularEncoding
from AbEgDiffuser.utils.protein.constants import BBHeavyAtom, AA
...
class ResidueEmbedding(nn.Module):
    ...
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, structure_mask=None, sequence_mask=None):
        ...
        crd = global_to_local(R, t, pos_atoms)    # (N, L, A, 3) 使用 construct_3d_basis 函数构建局部坐标系的旋转矩阵 R，并以 α 碳原子的坐标作为平移向量 t，将原子坐标从全局坐标系转换到局部坐标系。
        ...
        crd_feat = crd_expand.reshape(N, L, self.max_aa_types*self.max_num_atoms*3) # 将氨基酸类型和坐标特征进行扩展和掩码处理，得到形状为 (N, L, self.max_aa_types*self.max_num_atoms*3) 的坐标特征张量。
        if structure_mask is not None:
            # Avoid data leakage at training time
            crd_feat = crd_feat * structure_mask[:, :, None]

        # Backbone dihedral features
        bb_dihedral, mask_bb_dihed = get_backbone_dihedral_angles(pos_atoms, chain_nb=chain_nb, res_nb=res_nb, mask=mask_residue) # 计算主链二面角，并得到对应的掩码
