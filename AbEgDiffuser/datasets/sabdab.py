...
file_path = './data/RAbD_test.idx'
RAbD_PDB = []
with open(file_path, 'r') as file:
    for line in file:
        element = line.strip()
        RAbD_PDB.append(element)
...
class SAbDabDataset(Dataset):
    ...
    def __init__(
        self, 
        summary_path = '/root/autodl-tmp/data/sabdab/sabdab_summary_all.tsv',
        chothia_dir = '/root/autodl-tmp/data/sabdab/all_structures/chothia',
        ...
        self._load_split_rabd(split, split_seed)
    ...
    def _load_split_rabd(self, split, split_seed):
        assert split in ('train', 'val', 'test')
        ids_test = [
            entry['id']
            for entry in self.sabdab_entries
            if entry['id'] in RAbD_PDB
        ]
        test_relevant_clusters = set([self.id_to_cluster[id] for id in ids_test])
        ...
        if split == 'test':
            ...
            # self.ids_in_split = ids_train_val[:20]
            self.ids_in_split = ids_train_val[:int(0.1 * len(ids_train_val))]
        else:
            # self.ids_in_split = ids_train_val[20:]
            self.ids_in_split = ids_train_val[int(0.1 * len(ids_train_val)):]
...
def get_sabdab_dataset(cfg, transform):
    return SAbDabDataset(
        ...
        split = cfg.split,
        split_seed = cfg.get('split_seed', 2025),
        transform = transform,
    )
